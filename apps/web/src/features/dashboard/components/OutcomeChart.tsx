...
export const OutcomeChart = ({ data }: Props) => {
  ...
  if (!chartData.length) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '160px', fontSize: '14px', color: 'rgba(0,0,0,0.40)', letterSpacing: '-0.224px' }}>
        No data yet
      </div>
    )
  }
    ...
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 24, top: 0, bottom: 0 }}>
        <XAxis
          type="number"
          allowDecimals={false}
          tick={{ fontSize: 11, fill: 'rgba(0,0,0,0.40)', fontFamily: 'var(--font-sans,-apple-system)', letterSpacing: '-0.12px' }}
          axisLine={false}
          tickLine={false}
        />
        <YAxis
